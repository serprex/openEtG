...
var renderer = new PIXI.autoDetectRenderer(900, 600, {
	view: document.getElementById('leftpane'),
	transparent: true,
});
var noStage = {},
	curStage = noStage;
		...
		set: function(text) {
			...
			while ((reres = sep.exec(text))) {
				...
				if (reres.index != lastindex) {
					this.appendChild(
						document.createTextNode(text.slice(lastindex, reres.index)),
					);
				}
				if (piece == '\n') {
...
function DeckDisplay(decksize, cardmouseover, cardclick, deck) {
	...
	this.hitArea = new PIXI.math.Rectangle(
		100,
		32,
		Math.floor(decksize / 10) * 99,
		200,
	);
...
DeckDisplay.prototype.pos2idx = function() {
	return (
		Math.floor((exports.mouse.x - 100 - this.position.x) / 99) * 10 +
		Math.floor((exports.mouse.y - 32 - this.position.y) / 19) % 10
	);
};
...
DeckDisplay.prototype.addCard = function(code, i) {
	if (i === undefined) i = 0;
	for (; i < this.deck.length; i++) {
		if (Cards.codeCmp(this.deck[i], code) >= 0) break;
	}
...
CardSelector.prototype.click = function() {
	...
	var col = this.columns[Math.floor((exports.mouse.x - 100) / 133)],
		card;
...
CardSelector.prototype.renderColumns = function() {
	...
	for (var i = 0; i < 6; i++) {
		for (var j = 0; j < this.columns[i].length; j++) {
			var spr = this.columnspr[i][j],
				code = this.columns[i][j].code;
			...
			if (this.cardpool) {
				...
				var cardAmount =
					card.type == etg.PillarEnum
						? '-'
						: code in this.cardpool
							? this.cardpool[code] -
								((this.cardminus && this.cardminus[code]) || 0)
							: 0;
