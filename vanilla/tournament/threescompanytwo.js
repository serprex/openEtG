module.exports = function(deck){
	var cardtypes = [], typecount = 0, cardcount = {}, countcount = [];
	for(var i=0; i<deck.length; i++){
		...
		var type = Math.max(card == Cards.Chimera ? etg.CreatureEnum : card.type, etg.PermanentEnum);
		if (!cardtypes[type]){
			cardtypes[type]=1;
			typecount++;
			countcount[type]=0;
	...
	for (var key in countcount){
		if (countcount[key] != 3) return "Must include exactly 3 copies of type " + key;
	}
	var countsame = -1;
	for(var key in cardcount){
		if (countsame == -1) countsame = cardcount[key];
		else if (countsame != cardcount[key]) return "Card counts off: " + countsame + " vs " + cardcount[key] + " copies of " + Cards.Codes[key].name;
	}
	return "Legal";
}