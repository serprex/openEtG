...
module.exports = class Challenge extends preact.Component {
	render() {
		...
		const children = [h(Components.DeckDisplay, {deck:deck})],
			...
			pvphp = !self.state.challenge && h(Components.Input, { placeholder: "HP", opt: "pvphp", num: true, x: 190, y: 425 }),
			pvpmark = !self.state.challenge && h(Components.Input, { placeholder: "Mark", opt: "pvpmark", num: true, x: 190, y: 450 }),
			pvpdraw = !self.state.challenge && h(Components.Input, { placeholder: "Draw", opt: "pvpdraw", num: true, x: 190, y: 475 }),
			pvpdeck = !self.state.challenge && h(Components.Input, { placeholder: "Deck", opt: "pvpdeck", num: true, x: 190, y: 500 }),
			pvpButton = !self.state.challenge && h('input', {
				type: 'button',
				value: 'PvP',
			...
			spectateButton = !self.state.challenge && h('input', {
				type: 'button',
				value: 'Spectate',
				onClick: function() {
			...
			cancelButton = self.state.challenge && h('input', {
				type: 'button',
				value: 'Cancel',
				onClick: cancelClick,
				style: {
