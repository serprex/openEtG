...
fn eval_skill(
	...
	skills
		...
		.map(|&sk| match sk {
			...
			Skill::bblood => 7.0,
			Skill::beguilestop => -*damage.get(&c).unwrap_or(&0.0),
			Skill::bellweb => 1.0,
			Skill::blackhole => ctx
					...
					.map(|&perm| {
						if perm != 0 {
							if ctx.get(perm, Stat::pillar) != 0 {
								return if ctx.get_card(ctx.get(perm, Stat::card)).element as i32
									== etg::Chroma
								{
									3
								} else {
				...
				2.0 + ctx
					...
					.map(|&perm| {
						if perm != 0 {
							...
							{
								return (3 * ctx.get(perm, Stat::charges)) as f32;
							} else if (hasentropymark || element == etg::Entropy)
								&& ctx.hasskill(perm, Event::OwnAttack, Skill::pend)
							{
								return (((hasentropymark as i32)
									+ (element == etg::Entropy) as i32) * ctx
									.get(perm, Stat::charges)) as f32 * 1.5;
							}
						}
						0.0
					})
			...
			Skill::golemhit => {
				...
				for &cr in ctx.get_player(ctx.get_owner(c)).creatures.iter() {
					...
					{
						let atk = *damage.get(&cr).unwrap_or(&0.0);
						if atk > dmg {
							dmg = atk;
						}
					}
			...
			Skill::virtue => {
				if ctx.get_kind(c) == etg::Spell {
					...
					(ctx.trueatk(c) as f32 - *damage.get(&c).unwrap_or(&0.0)) / 1.5
				}
			}
			...
			Skill::firewall => {
				4.0 + ctx
					...
					.map(|&cr| {
						if cr != 0 {
							(cmp::max(6 - ctx.get(c, Stat::hp), 2) as f32).ln()
						} else {
							0.0
						}
					})
...
fn estimate_damage(ctx: &Game, id: i32, freedom: f32, wall: &mut Wall) -> f32 {
	...
	let mut atk = 0;
	for dmg in iter::once(tatk).chain(
...
fn evalthing(
	...
	let isattack = iscrea || kind == etg::Weapon;
	let mut score = 0.0;
	let mut delaymix = cmp::max(ctx.get(id, Stat::frozen), ctx.get(id, Stat::delayed)) as f32;
	let (ttatk, ctrueatk, adrenaline, delayfactor) = if isattack {
		let ttatk = *damage.get(&id).unwrap_or(&0.0);
		...
		score += ttatk + ctrueatk as f32 / (10.0 + delayfactor);
	...
	let mut hp = 0;
	if iscrea {
		if flooded && ctx.get(id, Stat::aquatic) == 0 && ctx.material(id, 0) && ctx.getIndex(id) > 4
		{
			hp = 0;
		} else {
			hp = cmp::max(ctx.truehp(id), 0);
			let poison = ctx.get(id, Stat::poison);
			if poison > 0 {
				hp = cmp::max(hp - poison * 2, 0);
				if ctx.get(id, Stat::aflatoxin) != 0 {
					score -= 2.0;
				}
			} else if poison < 0 {
				hp = cmp::max(cmp::min(hp - poison, ctx.get(id, Stat::maxhp)), 0);
			}
		}
		if patience {
			hp += 2;
		...
		if hp == 0 {
			...
			for j in 0..2 {
				...
				score += pl
					.creatures
					.iter()
					.chain(pl.permanents.iter())
					.map(|&r| {
						if r != 0 {
							eval_skill(ctx, r, ctx.getSkill(r, Event::Death), ttatk, &damage)
						} else {
							0.0
						}
					})
	...
	for (ev, sk) in ctx.iter_skills(id) {
		match ev {
			...
			Event::OwnAttack => {
				/* TODO legacy let's some procs through frozen */
...
pub fn eval(ctx: &Game) -> f32 {
	let turn = ctx.turn;
	let player = ctx.get_player(turn);
	if ctx.winner != 0 {
	...
	let mut walls: FxHashMap<i32, Wall> = Default::default();
	for j in 0..pcount {
		let pl = players[(p0 + j) % pcount];
		let shield = ctx.get_shield(pl);
		let mut wall = Wall {
			charges: 0,
			disentro: 0,
			dischroma: 0,
			dmg: 0,
			patience: player.permanents.iter().any(|&pr| {
				pr != 0
					&& ctx.get(pr, Stat::frozen) == 0
		...
		if shield != 0 {
			...
			{
				wall.disentro = ctx.get_player(pl).quanta(etg::Entropy) as i16;
			} else if ctx.hasskill(shield, Event::Shield, Skill::disfield)
				|| ctx.hasskill(shield, Event::Shield, Skill::v_disfield)
			{
				wall.dischroma = ctx.get_player(pl).quanta.iter().map(|&q| q as i16).sum();
			}
		}
		walls.insert(pl, wall);
	}
	...
	for j in 0..pcount {
		...
		let mut wall = walls.get_mut(&pl).unwrap();
		...
		let freedom = if ctx.cardset() == CardSet::Open {
			...
			cmp::min(
				player
					...
					.iter()
					.map(|&pr| {
						if pr != 0 && ctx.hasskill(pr, Event::Attack, Skill::v_freedom) {
							ctx.get(pr, Stat::charges)
						} else {
							0
						}
					})
		...
		if !stasis && !wall.patience {
			for &cr in player.creatures.iter() {
				if cr != 0 {
					let dmg = estimate_damage(ctx, cr, freedom, wall);
		...
		if weapon != 0 {
			let dmg = estimate_damage(ctx, weapon, freedom, wall);
			damage_hash.insert(weapon, dmg);
			total += dmg;
		}
		wall.dmg = total.min(32000.0).max(-32000.0) as i16;
	}
	if let Some(wall) = walls.get(&turn) {
		if wall.dmg as i32 > ctx.get(foe, Stat::hp) {
			return (wall.dmg as i32 - ctx.get(foe, Stat::hp) * 999) as f32;
		}
	}
	...
	for j in 0..pcount {
		...
		let expected_damage = players
			.iter()
			.filter(|&&p| ctx.get_foe(p) == pl)
			.map(|&p| walls.get(&p).map(|&w| w.dmg as i32).unwrap_or(0))
			.sum::<i32>() as f32;
		let player = ctx.get_player(pl);
		let wall = walls.get(&pl).unwrap();
		let mut pscore = (wall.charges * 4) as f32
			+ ((ctx.get(pl, Stat::markpower) + 1) as f32).ln()
			- expected_damage;
		let patience = wall.patience;
		if patience {
			pscore += (ctx.count_creatures(pl) * 2 + 2) as f32;
		}
		...
		if ctx.get(pl, Stat::neuro) != 0 {
			pscore -= (10 + player.hand.len()) as f32 / 2.0;
		}
