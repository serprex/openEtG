...
impl<'a> SkillThing<'a> {
	...
	fn sktext(&self, ev: Event, sk: Skill) -> Option<Cow<'static, str>> {
		Some(match sk {
			...
			Skill::absorber =>
				Cow::from("Generates 3:6 for each attacker"),
			Skill::acceleration =>
				Cow::from(format!("Replaces target creature's skills with \"Gains {} when it attacks",
								  if (*self).upped() {"+3|-1"} else {"+2|-1"})),
			Skill::accretion =>
				Cow::from(format!("Destroy target permanent & gain {}. If using this ability leaves this creature at more than {}, destroy this creature & add a black hole to your hand",
								  if (*self).set() == CardSet::Open { "0|10" } else { "0|15" },
								  if (*self).set() == CardSet::Open { "30HP" } else { "45HP" })),
			Skill::accumulation =>
				Cow::from("Playing additional Repulsors adds to damage reduction"),
			Skill::adrenaline =>
				Cow::from("Target creature attacks multiple times per turn. Creatures with lower strength attack more times per turn"),
			Skill::aflatoxin =>
				Cow::from("Give target 2 poison. When the target dies, it becomes a Malignant Cell"),
			Skill::aggroskele =>
				Cow::from(format!("Summon a {} Skeleton. All of your skeletons deal damage equal to their strength to target creature",
								  if (*self).upped() { "2|2" } else { "1|1" })),
			Skill::alphawolf =>
				Cow::from("Summon two 2|1 Pack Wolves when this enters play"),
			Skill::antimatter =>
			...
			Skill::axe =>
				Cow::from("Gain 1 strength if your mark is 1:6 1:10"),
			Skill::axedraw =>
				Cow::from("Gains 1 strength for every card drawn by any player. Strength gained is removed after attack"),
			Skill::bblood =>
				Cow::from("Give target creature 0|20 & delay it for 5 turns"),
			Skill::becomearctic =>
				Cow::from("If frozen, this creature instead turns into an Arctic Squid"),
			Skill::beguile =>
			...
			Skill::bellweb =>
				Cow::from("Target creature becomes aquatic & loses airborne status"),
			Skill::blackhole =>
				Cow::from("Remove 3 quanta per element from target player. Heal 1 per quanta removed"),
			Skill::bless =>
				Cow::from("Target gains 3|3"),
			Skill::blockhp =>
				Cow::from("Gain maxhp equal to amount of damage blocked"),
			Skill::blockwithcharge =>
				Cow::from(format!("Each stack fully blocks one attacker{}",
								  if (*self).set() == CardSet::Open { " & is then destroyed" } else { "" })),
			Skill::bloodmoon =>
				Cow::from("Aquatic creatures gain \"Gain 1:8 when it attacks.\"\nGolems gain \"Damage dealt by this card also reduces the defender's maximum HP.\"\nNocturnal creatures gain \"Heal yourself equal to the damage dealt by this card.\""),
			Skill::bolsterintodeck =>
				Cow::from("Add 3 copies of target creature on top of your deck"),
			Skill::boneyard =>
				Cow::from(format!("Whenever a creature which isn't a Skeleton dies, summon a {} Skeleton",
								  if (*self).upped() { "2|2" } else { "1|1" })),
			Skill::bounce =>
				Cow::from("When dying instead return to owner's hand. Modified state besides this effect remains when played again"),
			Skill::bow =>
				Cow::from(format!("Gain 1 strength if your mark is {}",
								  if (*self).set() == CardSet::Open { "1:8 1:9" } else { "1:9" })),
			Skill::bravery =>
				Cow::from(format!("You & opponent draw up to {} cards.",
								  if (*self).set() == CardSet::Original {"2 (3 if your mark is 1:6) "} else {"2"})),
			Skill::brawl =>
				Cow::from("Your creatures attack. If a creature exists in opposing creature slot, the two creatures deal their damage to one another instead of opponent. Consumes all 1:3"),
			Skill::brew =>
				Cow::from("Add a random Alchemy card to your hand. Possible cards are Antimatter, Black Hole, Adrenaline, Nymph's Tears, Unstable Gas, Liquid Shadow, Aflatoxin, Basilisk Blood, Rage Potion, Luciferin, Precognition, Quintessence"),
			Skill::brokenmirror =>
				Cow::from(format!("When opponent plays a creature from their hand, summon a {} Phantom",
								  if (*self).upped() { "2|1" } else { "1|1" })),
			Skill::bubbleclear =>
			...
			Skill::burrow =>
				Cow::from(if (*self).get_flag(Flag::burrowed) {
					"Unburrow this creature"
				} else {
					"Burrow this creature. Strength is halved while burrowed"
				}),
			Skill::catapult =>
				Cow::from("Sacrifice target creature you control to damage opponent for 100 * Creature's HP / (100 + Creature's HP). Frozen creatures deal 1.5x more. Poisoned creatures transfer their poison to opponent"),
			Skill::catlife => {
				let card = self.card();
			...
			Skill::cell =>
				Cow::from("Becomes a Malignant Cell if poisoned"),
			Skill::chaos =>
				Cow::from("Non-ranged attacking creatures have a 30% chance to have a random effect cast on them"),
			Skill::chimera =>
			...
			Skill::clear =>
				Cow::from("Remove statuses (positive & negative) from target creature or permanent, & heal target creature 1"),
			Skill::cold =>
				Cow::from("30% chance to freeze non-ranged attackers for 3 turns"),
			Skill::corpseexplosion =>
				Cow::from(format!("Sacrifice one of your creatures to deal 1 spell damage to all {} creatures. Increase damage by 1 for every 8HP of the sacrifice. Poisonous sacrifices poison. Also affect opponent",
								  if (*self).upped() {"enemy"} else {"other"})),
			Skill::counter =>
			...
			Skill::countimmbur =>
				Cow::from("Gains 1|0 for every immaterial or burrowed card in play"),
			Skill::cpower =>
				Cow::from("Target randomly gains between 1 to 5 strength & HP"),
			...
			Skill::dagger =>
				Cow::from("Gain 1 strength if your mark is 1:2 1:11. Gain 1 strength per Darkness or Death non-pillar permanent you control"),
			Skill::deadalive => {
				if ev == Event::Hit {
					Cow::from("When this card deals damage, trigger all effects that occur when a creature dies")
				}
				else if ev == Event::Cast {
					Cow::from("Trigger all effects that occur when a creature dies")
				}
			},
			Skill::deathwish =>
				Cow::from("Whenever opponent casts a spell or ability on an allied creature, if this creature is a valid target, that spell or ability targets this creature instead"),
			Skill::deckblast =>
			...
			Skill::deckblock =>
				Cow::from("Block all damage from attackers by destroying your deck's top card when attacked"),
			Skill::deepdive =>
			...
			Skill::deja =>
				Cow::from("Remove this ability & summon a copy of this creature"),
			Skill::deployblobs =>
				Cow::from("Summon 3 Blobs. Gain -2|-2"),
			Skill::despair =>
				Cow::from("Non-ranged attackers have a 50% chance plus 1% per nocturnal creature you control to gain -1|-1"),
			Skill::destroy =>
				Cow::from("Destroy target permanent"),
			Skill::destroycard =>
			...
			Skill::disc =>
				Cow::from("Gain 1 strength if your mark is 1:1 1:12"),
			Skill::discping =>
			...
			Skill::disfield =>
				Cow::from("Block all damage from attackers. Consumes 1:0 per damage blocked"),
			Skill::dispersion =>
				Cow::from("Discard target card in your hand. If card is a spell with targeting, cast on all available targets in random order. Each cast costs cost of card"),
			Skill::disshield =>
				Cow::from("Block all damage from attackers. Consume 1:1 per 3 damage blocked"),
			Skill::divinity =>
				Cow::from("Add 24 to maximum health & heal yourself 16"),
			Skill::dive =>
				Cow::from(if (*self).set() == CardSet::Open {
			...
			Skill::dmgproduce =>
				Cow::from("Generate 1:0 for each damage dealt by this card"),
			Skill::doctor =>
				Cow::from("Remove target's poison. When destroyed, give opponent that poison"),
			Skill::draft =>
				Cow::from("If target creature is airborne, it loses airborne & takes 3 spell damage. If target creature isn't airborne, it becomes airborne & gains 3|0"),
			Skill::drainlife =>
			...
			Skill::drawequip =>
				Cow::from("Both players draw the next weapon or shield in their deck"),
			Skill::drawpillar =>
			...
			Skill::dshield =>
				Cow::from("Target creature gains immaterial until next turn"),
			Skill::duality =>
				Cow::from("Add a copy of top card of opponent's deck to your hand"),
			Skill::earthquake(x) =>
				Cow::from(format!("Destroy up to {x} copies of target {}",
								  if (*self).set() == CardSet::Open {"stacking permanent"} else {"pillar"})),
			Skill::eatspell =>
			...
			Skill::enchant =>
				Cow::from("Target permanent becomes immaterial"),
			Skill::endow =>
			...
			Skill::evade(x) =>
				Cow::from(format!("{x}% chance to evade attacks")),
			Skill::evade100 =>
				Cow::from("Completely block enemy attacks"),
			Skill::evadecrea =>
				Cow::from("Cannot be directly targeted by opponent's creature's active skills"),
			Skill::evadespell =>
				Cow::from("Cannot be directly targeted by opponent's spells"),
			Skill::evolve =>
				Cow::from(format!("Transform this card into an unburrowed {} Shrieker",
								  if (*self).upped() {"10|4"} else {"8|3"})),
			Skill::feed =>
				Cow::from("Poison other target creature, gain 3|3, & lose immaterial"),
			Skill::fickle =>
			...
			Skill::fiery =>
				Cow::from("Gains 1 strength for every 5:6 in your quanta pool"),
			Skill::firebolt =>
			...
			Skill::firebrand =>
				Cow::from("Last an additional turn when targeted with Tempering"),
			Skill::firestorm(x) =>
				Cow::from(format!("Deal {} spell damage to all of target player's creatures, thawing them. Removes cloak", x)),
			Skill::firewall =>
				Cow::from("Deals 1 damage to each non-ranged attacking creature"),
			Skill::flooddeath =>
			...
			Skill::foedraw =>
				Cow::from("Draw from opponent's deck"),
			Skill::forcedraw =>
				Cow::from("When this creature damages a player, that player draws a card"),
			Skill::forceplay =>
			...
			Skill::frail =>
				Cow::from("Target creature's hp/maxhp becomes 1 without receiving damage, or target permanent with more than 1 charge/stack has one charge/stack removed"),
			Skill::fractal =>
			...
			Skill::freeze(x) =>
				Cow::from(format!("Freeze target creature or weapon for {x} turns. Frozen cards cannot attack or use active skills & do not activate per-turn skills")),
			Skill::freezeperm =>
				Cow::from(format!("Freeze target non-stacking permanent for {} turns. Frozen cards cannot attack or use active skills & do not activate per-turn skills",
								  if (*self).upped() {'4'} else {'3'})),
			Skill::fungusrebirth =>
				Cow::from(format!("Transform this card into a {}",
								  if (*self).upped() {"Toxic Fungus"} else {"Fungus"})),
			Skill::gaincharge2 => {
				if ev == Event::Death {
					Cow::from("Whenever any creature dies, gain two stacks")
				}
				else if ev == Event::Destroy {
			...
			Skill::gas =>
				Cow::from("Summon an Unstable Gas"),
			Skill::grave =>
			...
			Skill::give =>
				Cow::from(format!("Give target card you own, either in hand or in play, to your opponent. Heal yourself {}. This card bypasses sanctuary & can target immaterial or burrowed cards",
			...
			Skill::growth(atk, hp) => {
				if ev == Event::Death {
					Cow::from(format!("When any creature dies, gain {atk}|{hp}"))
				}
				else if ev == Event::Cast {
					Cow::from(format!("Gain {atk}|{hp}"))
				}
				else if ev == Event::OwnAttack {
					Cow::from(format!("This creature gains {atk}|{hp} when it attacks"))
				}
			},
			Skill::icegrowth(atk, hp) =>
				Cow::from(format!("When this card would be frozen, instead gain {atk}|{hp}")),
			Skill::guard =>
			...
			Skill::halveatk =>
				Cow::from("This creature's strength is halved after it attacks"),
			Skill::halvedr =>
				Cow::from("Halve damage reduction at start of your turn"),
			Skill::hammer => Cow::from("Gain 1 strength if your mark is 1:3 1:4"),
			Skill::hasten => {
				let mut s = String::new();
				match ev {
					Event::Cast => s.push_str("Draw a card"),
					Event::OwnDiscard => s.push_str("When discarded, you draw a card"),
					_ => return None,
				}
				Cow::from(s)
			},
			...
			Skill::heal =>
				Cow::from("Heal target creature or player 20"),
			Skill::heatmirror =>
				Cow::from(format!("When your opponent plays a creature from their hand, summon a {}",
								  if (*self).upped() {"Ball Lightning"} else {"Spark"})),
			Skill::hitownertwice =>
				Cow::from("When this creature attacks, it also attacks its owner twice"),
			Skill::holylight =>
			...
			Skill::ignitediscard =>
				Cow::from("When discarded, deal 5 spell damage to opponent"),
			Skill::immolate(x) =>
			...
			Skill::inertia =>
				Cow::from("When any card you own is targeted by either player, gain 2:3"),
			Skill::inflation =>
				Cow::from("Increase the cost of all active skills by 1"),
			Skill::ink =>
				Cow::from("Summon a Cloak that lasts 1 turn"),
			Skill::innovation =>
			...
			Skill::lightning =>
				Cow::from("Deal 5 spell damage to target creature or player"),
			Skill::liquid =>
			...
			Skill::lobotomize =>
				Cow::from("Remove target creature's skills. Also remove psionism"),
			Skill::locket =>
			...
			Skill::loot =>
				Cow::from("When one of your permanents is destroyed, gain control of a random permanent from opponent"),
			Skill::losecharge => {
			...
			Skill::mend =>
				Cow::from(format!("Heal target creature {}",
								  if (*self).set() == CardSet::Open {"10"} else {"5"})),
			Skill::metamorph =>
			...
			Skill::mill =>
				Cow::from("Destroy top card of target player's deck"),
			Skill::millpillar =>
			...
			Skill::mist =>
				Cow::from("Each stack has a 5% chance to evade attacks"),
			Skill::miracle =>
				Cow::from("Heal yourself to one below your maximum HP. Consumes all 1:8"),
			Skill::mitosis =>
				Cow::from("Summon a fresh copy of this creature"),
			Skill::mitosisspell =>
			...
			Skill::momentum =>
				Cow::from("Target creature or weapon gains 1|1 & ignores shields"),
			Skill::mummy =>
				Cow::from("Becomes a Pharaoh if targeted by Rewind"),
			Skill::mutation =>
			...
			Skill::nightmare =>
				Cow::from(format!("Fill opponent's hand with fresh copies of target creature. Deal {} damage per card added in this way. Heal yourself an equal amount",
								  if (*self).set() == CardSet::Open && !(*self).upped() {"1"} else {"2"})),
			Skill::nightshade =>
				Cow::from("Target creature becomes nocturnal, gains 5|5, & loses active skills"),
			Skill::nothrottle =>
				Cow::from("Your creatures whose skills have Throttled lose Throttled"),
			Skill::nova =>
				Cow::from(format!("Gain {} quanta of each element. If you play {} or more of this card in one turn, summon a Singularity on your side", "1", "3")),
			Skill::nova2 =>
				Cow::from(format!("Gain {} quanta of each element. If you play {} or more of this card in one turn, summon a Singularity on your side", "2", "2")),
			Skill::nullspell =>
				Cow::from("Until your next turn, the next spell any player casts is nullified. If this ability nullifies a spell, this creature gains 1|1"),
			Skill::nymph =>
				Cow::from("Transform target pillar, pendulum, or tower into a Nymph matching target's element"),
			Skill::obsession =>
				Cow::from(format!("When discarded, its owner receives {} spell damage",
								  if (*self).upped() {"13"} else {"10"})),
			Skill::ouija =>
				Cow::from("Whenever a creature dies, add an Ouija Essence to opponent's hand"),
			Skill::ouijadestroy =>
				Cow::from("When destroyed, add 1 to opponent's maximum health"),
			Skill::ouijagrowth =>
				Cow::from("Summon an Ouija Essence on opponent's side of the field"),
			Skill::pacify =>
				Cow::from("Set target creature or weapon's strength to 0"),
			Skill::pairproduce =>
				Cow::from("Your pillars, pendulums, & towers trigger as if end of turn"),
			Skill::paleomagnetism =>
				Cow::from(format!("Summon a {} or pendulum every turn. \u{2154} chance it matches your mark, otherwise it matches your opponent's mark",
								  if (*self).upped() {"tower"} else {"pillar"} )),
			Skill::pandemonium =>
				Cow::from(format!("Apply a random effect to {}. Possible effects include damage, lobotomize, parallel universe, gravity pull, rewind, & freeze. Removes cloak", "each creature in play")),
			Skill::pandemonium2 =>
				Cow::from(format!("Apply a random effect to {}. Possible effects include damage, lobotomize, parallel universe, gravity pull, rewind, & freeze. Removes cloak", "each of target player's creatures")),
			Skill::pandemonium3 =>
				Cow::from(format!("Apply a random effect to {}. All existing effects are possible. Removes cloak", "every card currently in play and to every card held by both players")),
			Skill::paradox =>
				Cow::from("Target creature with more strength than HP dies"),
			Skill::parallel =>
				Cow::from("Summon an exact copy of target creature on your side"),
			Skill::patience =>
			...
			Skill::phoenix =>
				Cow::from(format!("When this creature dies, transform it into {}",
								  if (*self).upped() {"a Minor Ash"} else {"an Ash"})),
			Skill::photosynthesis =>
				Cow::from("Gain 2:5. Unless cost was 0 or 1:0, caster can be reactivated"),
			Skill::pillar =>
				Cow::from(format!("Gain {}:{} every turn",
								  if (*self).card().element == 0 { '3' } else { '1' },
								  self.card().element)),
			Skill::pillar1 =>
				Cow::from(format!("Gain {}:{} when played",
								  if (*self).card().element == 0 { '3' } else { '1' },
								  self.card().element)),
			Skill::pend =>
				Cow::from(format!("Each turn, switch between gaining {}:{} & one quanta matching your mark",
								  if (*self).card().element == 0 { '3' } else { '1' },
								  self.card().element)),
			Skill::plague =>
			...
			Skill::poisondr =>
				Cow::from("Reduce damage by amount attacker is poisoned. Weapon damage reduced by poison of wielder"),
			Skill::poisonfoe(x) => {
				let mut s = String::new();
				s.push_str(match ev {
			...
			Skill::quint =>
				Cow::from("Target creature becomes immaterial and loses frozen status"),
			Skill::quinttog =>
				Cow::from("Target creature becomes immaterial and loses frozen status, or loses immaterial if it already has this status"),
			Skill::rage => {
			...
			Skill::randomdr =>
				Cow::from(format!("When this card is played, its damage reduction is set randomly between 0 & {}",
								  if (*self).upped() { '3' } else { '2' })),
			Skill::readiness =>
			...
			Skill::reap =>
				Cow::from("Target non-Skeleton creature dies & is replaced with a Skeleton with target creature's current strength & HP, but no other active skills or statuses"),
			Skill::rebirth =>
				Cow::from(format!("Transform this card into a {}",
								  if (*self).upped() {"Minor Phoenix"} else {"Phoenix"})),
			Skill::reducemaxhp =>
			...
			Skill::regenerate(x) =>
				Cow::from(format!("Heal yourself {x} every turn or when this card attacks")),
			...
			Skill::ren =>
				Cow::from("Target creature gains: \"When dying instead return to owner's hand. Modified state besides this effect remains when played again.\""),
			Skill::resetdr =>
				Cow::from("Zero damage reduction at start of your turn"),
			Skill::resummon =>
				Cow::from("Target creature is summoned again as is"),
			Skill::rewind =>
				Cow::from("Put target creature on top of its owner's deck. Removes all bonuses & modifiers on target creature"),
			Skill::reveal if ev == Event::OwnPlay =>
				Cow::from("Reveal opponent's hand when played & on attack"),
			Skill::ricochet =>
			...
			Skill::sadism =>
				Cow::from("Whenever any creatures are damaged, heal yourself an equal amount"),
			Skill::salvage =>
				Cow::from(if (*self).set() == CardSet::Open {
			...
			Skill::salvageoff =>
				Cow::from("Cannot salvage another destroyed permanent until next turn"),
			Skill::sanctify if ev == Event::OwnAttack =>
				Cow::from("During your opponent's turn, your hand & quanta pool are protected & you cannot be silenced"),
			Skill::sanctify if ev == Event::OwnDraw =>
				Cow::from("When drawn, your hand & quanta pool are protected & you cannot be silenced"),
			Skill::unsanctify if ev == Event::OwnPlay =>
				Cow::from("Nullify opponent's sanctuary effect"),
			Skill::scatter =>
			...
			Skill::scramble if ev == Event::Cast =>
				Cow::from("Randomize up to 9 quanta randomly chosen from target player's quanta pool"),
			Skill::scramblespam =>
			...
			Skill::serendipity =>
				Cow::from(format!("Add 3 random {} cards to your hand. At least one will be 1:1",
								  if (*self).upped() {"upgraded non-pillar"} else {"non-pillar"})),
			Skill::shtriga =>
				Cow::from("Gain immaterial when your next turn starts"),
			Skill::shuffle3 =>
			...
			Skill::sing =>
				Cow::from("Target creature without this ability attacks its owner"),
			Skill::singularity =>
				Cow::from("That was a bad idea"),
			Skill::sinkhole =>
				Cow::from(format!("Burrow target creature. Replace target creature's skills with {}",
								  if (*self).upped() {"2:4: unburrow"} else {"1:4: unburrow"})),
			Skill::siphon =>
			...
			Skill::siphonstrength =>
				Cow::from("Target creature loses 1|0. Gain 1|0"),
			Skill::skeleton =>
			...
			Skill::slime =>
				Cow::from("Increase damage reduction by 1 when damage reduction does not prevent all damage from non-ranged attacker"),
			Skill::slow =>
				Cow::from("Non-ranged attackers are delayed for one turn after their attack. Delayed creatures may not attack or use active skills"),
			Skill::snipe =>
				Cow::from("Deal 3 damage to target creature"),
			Skill::solar =>
				Cow::from("Gain 1:8 for each attacker"),
			Skill::sosa =>
				Cow::from(format!("Sacrifice {}. Consume all non-1:2 quanta. For two turns, damage heals you & healing damages you",
								  if (*self).upped() {"40HP"} else {"48HP"})),
			Skill::soulcatch =>
				Cow::from(format!("Whenever a creature dies, gain {}",
								  if (*self).set() == CardSet::Original && !self.upped() {"2:2"} else {"3:2"})),
			Skill::spores =>
				Cow::from(format!("When this creature dies, summon 2 {}",
								  if (*self).upped() {"Toxic Spores"} else {"Spores"})),
			Skill::sskin =>
				Cow::from("Gain maximum HP & heal an amount equal to the 1:4 in your quanta pool after casting this spell"),
			Skill::staff =>
				Cow::from("Gain 1 strength if your mark is 1:5 1:7"),
			Skill::stasis =>
				Cow::from("Creatures do not attack at the end of each player's turn"),
			Skill::stasisdraw =>
			...
			Skill::r#static(x) =>
				Cow::from(format!("Deals {x} spell damage to opponent for each attacker")),
			Skill::steal =>
				Cow::from("You gain control of target permanent"),
			Skill::steam =>
				Cow::from("Gain 5|0. This creature loses 1|0 of strength gained in this way after each attack"),
			Skill::stoneform =>
				Cow::from("Gain 0|20. Become a golem"),
			Skill::stonewall =>
				Cow::from("Gain 1 damage reduction for every 9:4 in your quanta pool"),
			Skill::storm(x) =>
			...
			Skill::tesseractsummon =>
				Cow::from("Summon a random creature from opponent's deck. Summon two random creatures from your deck. Freeze these creatures for a number of turns equal to \u{00bc} of their quanta cost, rounded up. Freeze Tesseract for two turns"),
			Skill::thorn(x) =>
				Cow::from(format!("{x}% chance to poison non-ranged attackers")),
			Skill::throwrock =>
				Cow::from(format!("Deal {} damage to target creature, then shuffle Throw Rock into its owner's deck",
								  if (*self).upped() {"4"} else {"3"})),
			Skill::tick =>
				Cow::from(format!("This creature takes {} damage. If this damage kills the creature, deal {}",
								  if (*self).upped() {"3"} else {"1"},
								  if (*self).upped() {"4 spell damage to all of opponent's creatures"} else {"18 spell damage to opponent"})),
			Skill::tidalhealing =>
			...
			Skill::tornado =>
				Cow::from(format!("Randomly choose {}. Each selected permanent is shuffled into a random player's deck",
								  if (*self).upped() {"two of opponent's permanents"} else {"two of opponent's permanents & one of your permanents"})),
			Skill::trick =>
			...
			Skill::tutordraw =>
				Cow::from("Search deck for copy of target card. Draw that copy"),
			Skill::turngolem =>
			...
			Skill::unsilence =>
				Cow::from("Remove silence from yourself"),
			Skill::unsummon =>
				Cow::from(if (*self).upped() {
			...
			Skill::upload =>
				Cow::from("Target creature or weapon gains 2|0. This creature loses 0|2"),
			Skill::vampire =>
				Cow::from("Heal yourself equal to the damage dealt by this card"),
			Skill::vend =>
				Cow::from("Sacrifice this card. Draw a card"),
			Skill::vengeance =>
			...
			Skill::void =>
				Cow::from(format!("Reduce {} maximum HP by {}",
								  if (*self).set() == CardSet::Open {"opponent's maximum HP by 3"} else {"foe's"},
								  if (*self).set() == CardSet::Open {"2"} else {"2, 3 if mark is 1:11"})),
			Skill::voidshell =>
				Cow::from("Block all damage from attackers. Reduce your maximum HP equal to the damage blocked by this card"),
			Skill::web =>
				Cow::from("Target creature loses airborne status"),
			Skill::weight =>
				Cow::from("Evade all attackers that have more than 5HP"),
			Skill::wicked =>
				Cow::from("Sacrifice creature you control. Increase damage reduction by that creature's cost"),
			Skill::wind =>
				Cow::from("Restore any strength lost by halving after attacking. Increase HP by amount restored"),
			Skill::wings =>
				Cow::from("Evade all non-airborne, non-ranged attackers"),
			Skill::wisdom =>
				Cow::from(format!("Target {}. May target immaterial cards. If it targets an immaterial card, that card gains psionic. Psionic cards deal spell damage & typically bypass shields",
								  if (*self).set() == CardSet::Open {"gains 3|0"} else {"creature gains 4|0"})),
			Skill::yoink =>
				Cow::from("Remove target card from opponent's hand & add it to your hand, or draw from target opponent's deck"),
			Skill::v_bblood =>
				Cow::from("Target creature gains 0|20 & is delayed 6 turns"),
			Skill::v_blackhole =>
				Cow::from("Absorb 3 quanta per element from target player. Heal 1 per absorbed quantum"),
			Skill::v_cold =>
				Cow::from("30% chance to freeze attackers for 3"),
			Skill::v_cseed =>
				Cow::from("A random effect is inflicted to target creature"),
			Skill::v_dessication =>
				Cow::from("Deal 2 damage to opponent's creatures. Gain 1:7 per damage dealt. Removes cloak"),
			Skill::v_divinity =>
				Cow::from("Add 24 to maximum health if mark 1:8, otherwise 16 & heal same"),
			Skill::v_drainlife(..) =>
				Cow::from("Drain 2HP from target, plus an extra 2HP per 10:11 remaining"),
			Skill::v_dshield =>
				Cow::from("Become immaterial until next turn"),
			Skill::v_endow =>
				Cow::from("Replicate attributes of target weapon"),
			Skill::v_firebolt(..) =>
				Cow::from("Deals 3 damage to target. Deal 3 more per 10:6 remaining"),
			Skill::v_firewall =>
				Cow::from("Damage attackers"),
			Skill::v_flyingweapon =>
				Cow::from("Own weapon becomes a flying creature"),
			Skill::v_freedom =>
				Cow::from("Your airborne creatures have a 25% chance to deal 50% more damage, bypass shields & evade targeting if 1:9"),
			Skill::v_gratitude =>
				Cow::from("Heal owner 3, 5 if your mark is 1:5"),
			Skill::v_hatch =>
				Cow::from("Become a random creature"),
			Skill::v_heal =>
				Cow::from("Heal self 20"),
			Skill::v_holylight =>
				Cow::from("Heal target 10. Nocturnal targets are damaged instead"),
			Skill::v_hope =>
				Cow::from("Blocks one additional damage for each creature you control that gain 1:8 when attacking"),
			Skill::v_icebolt(..) =>
			...
			Skill::v_improve =>
				Cow::from("Mutate target creature"),
			Skill::v_integrity =>
				Cow::from("Combine all shards in hand to form a Shard Golem"),
			Skill::v_mutation =>
				Cow::from("Mutate target creature into an abomination, or maybe something more. Slight chance of death"),
			Skill::v_nymph =>
				Cow::from("Turn target pillar into a Nymph of same element"),
			Skill::v_obsession =>
				Cow::from(format!("Damage owner {} on discard",
								  if (*self).upped() {"13"} else {"10"})),
			Skill::v_pandemonium =>
				Cow::from(format!("Random effects are inflicted to {}. Removes cloak",
								  if (*self).upped() {"opponent's creatures"} else {"all creatures"})),
			Skill::v_plague =>
				Cow::from("Poison foe's creatures. Removes cloak"),
			Skill::v_readiness =>
			...
			Skill::v_scramble =>
				Cow::from("Randomly scramble foe's quanta on hit"),
			Skill::v_serendipity =>
				Cow::from(format!("Generate 3 random {} in hand. One will be 1:1",
								  if (*self).upped() {"upgraded cards"} else {"cards"})),
			Skill::v_silence =>
				Cow::from("Foe cannot play cards during their next turn, or target creature gains summoning sickness"),
			Skill::v_singularity =>
				Cow::from("Not well behaved"),
			Skill::v_slow =>
				Cow::from("Delay attackers"),
			Skill::v_steal =>
				Cow::from("Steal target permanent"),
			Skill::v_stoneform =>
				Cow::from("Remove this ability & gain 0|20"),
			Skill::v_storm(x) =>
				Cow::from(format!("Deals {x} damage to foe's creatures. Removes cloak")),
			Skill::v_swarm =>
				Cow::from("Increment hp per scarab"),
			Skill::v_thorn =>
				Cow::from("75% chance to poison attackers"),
			Skill::v_virusplague =>
				Cow::from("Sacrifice self & poison foe's creatures"),
			Skill::v_dagger =>
				Cow::from("Gain 1 strength if your mark is 1:2 1:11"),
			_ => return None
		})
	}
