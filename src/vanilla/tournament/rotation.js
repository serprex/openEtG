export default function(deck, mark) {
	...
	for (var i = 0; i < deck.length; i++) {
		...
		if (card.element && !~elementCount.indexOf(card.element))
			elementCount.push(card.element);
		if (card.element == mark) {
			if (card.type == etg.PillarEnum) {
				markPillars++;
				if (markPillars > 4)
					return "You have too many pillars/pends/mark cards of your mark's element";
			} else {
				markCards.push(card.name);
				if (markCards.length > 7)
					return "You have too many cards of your mark's element, 7 is the maximum";
			}
		}
	}
	if (markCards.length < 4)
		return "You have too few cards of your mark's element, 4 is the minimum";
