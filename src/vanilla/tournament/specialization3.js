export default function(deck) {
	...
	for (var i = 0; i < deck.length; i++) {
		...
		if (cardList[card.asUpped(false).code])
			cardList[card.asUpped(false).code]++;
	...
	if (upped < 0)
		return 'You have too few unupgraded cards, needs ' + -upped + ' more';
	...
	for (var code in cardList) {
		...
		if (~specialList.indexOf(code)) {
			if (cardList[code] < 6)
				return (
					'You have to have at least 6 copies of ' + Cards.Codes[code].name
				);
			hasSpecial = true;
		} else {
			if (cardList[code] > 4 && card.type)
				return 'You have too many copies of ' + Cards.Codes[code].name;
		}
	}
	if (!hasSpecial)
