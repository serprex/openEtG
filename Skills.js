...
function adrenathrottle(f) {
	return (c, t, data) => {
		if (
			...
			(c.type == etg.Creature &&
				c.owner.weapon &&
				c.owner.weapon.status.get('nothrottle'))
		) {
			return f(c, t, data);
		}
	};
...
const Skills = (module.exports = {
	...
	freeevade: (c, t, data) => {
		...
		if (
			...
			tgt.owner == c.owner &&
			tgt.owner != t.owner &&
			tgt.status.get('airborne') &&
			!tgt.status.get('frozen') &&
			c.rng() > 0.8
		) {
			data.evade = true;
		}
	...
	phoenix: (c, t, data) => {
		if (!c.owner.creatures[data.index]) {
			var ash = (c.owner.creatures[data.index] = new Thing(
				c.card.as(Cards.Ash),
			));
			ash.owner = c.owner;
	...
	powerdrain: (c, t) => {
		...
		if (!ti.length) return;
		var tgt = c.owner.creatures[c.choose(ti)],
	...
	reap: (c, t) => {
		...
		if (
			...
			var skele = (t.owner.creatures[index] = new Thing(
	...
	serendipity: c => {
		var num = Math.min(8 - c.owner.hand.length, 3),
			anyentro = false;
		for (var i = num - 1; ~i; i--) {
			var card = c.randomcard(c.card.upped, x => {
				return (
					x.type != etg.Pillar &&
					x.rarity < 4 &&
	...
	tidalhealing: (c, t) => {
		c.owner.masscc(c, (c, t) => {
			...
			if (t.status.get('aquatic') && !t.hasactive('hit', 'regen'))
	...
	tornado: (c, t) => {
		...
		for (var i = 0; i < 3; i++) {
			...
			var perms = pl.permanents.filter(x => {
				return x && x.isMaterial();
			});
	...
	cold: (c, t) => {
		if (!t.status.get('ranged') && c.rng() < 0.3) {
			t.freeze(3);
		}
	},
	despair: (c, t) => {
		if (!t.status.get('ranged')) {
			var chance = c.owner.creatures.reduce((chance, cr) => {
	...
	thorn: (c, t) => {
		if (!t.status.get('ranged') && c.rng() < 0.75) {
			t.addpoison(1);
		}
	},
	thornweak: (c, t) => {
		if (!t.status.get('ranged') && c.rng() < 0.25) {
			t.addpoison(1);
		}
	},
	weight: (c, t, data) => {
