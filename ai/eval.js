...
const SkillsValues = Object.freeze({
	...
	brokenmirror: function(c) {
		return c.owner.foe.shield && c.owner.foe.shield.status.get('reflective')
			? -3
			: 2;
	},
	burrow: 1,
	...
	mutation: 4,
	neuro: function(c) {
		return c.owner.foe.status.get('neuro')
...
function evalthing(c) {
	...
	var ttatk,
		...
		poison,
		score = 0;
	var isCreature = (c.type == etg.Spell ? c.card.type : c.type) == etg.Creature,
		isAttacker =
			isCreature || (c.type == etg.Spell ? c.card.type : c.type) == etg.Weapon;
	if (isAttacker) {
		var ctrueatk = c.trueatk();
		var adrenalinefactor = c.status.get('adrenaline')
