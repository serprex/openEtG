...
function deckRedirect(req, res, next){
	...
	var code = req.url.slice(1, 4).replace(/\.svg$/, "");
	if (!(code in Cards.Codes)) return next();
	...
	readStream.on("error", function(err, data){
		...
		ret += "<image xlink:href='../Cards/"+etgutil.asShiny(etgutil.asUpped(code, false), false)+".png' y='20px' width='128px' height='128px'/>";
