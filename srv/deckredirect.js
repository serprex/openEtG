	...
	fs.readFile("deckcache/" + deck, function(err, data){
		if (!err){
			...
			etgutil.iterdeck(deck, function(code){
				...
				ret += "<rect x='"+x+"' y='"+y+"' width='100' height='16' fill='"+elecols[card.element+(card.upped?13:0)]+"' class='c'/><text x='" + (x+2) + "' y='" + (y+13) + "'" + textColor + ">" + card.name + "</text>";
				y += 16;
				if (y == 160){
					y=0;
					x+=100;
			...
			ret = "<svg xmlns='http://www.w3.org/2000/svg' height='160'"+" width='"+(y?x+100:x)+"'><style type='text/css'><![CDATA[text{font-size:12px;}.c{stroke:black;stroke-width:.5;}]]></style>" + ret + "</svg>";
			res.writeHead(200, {"Content-Type": "image/svg+xml"});
			res.end(ret);
