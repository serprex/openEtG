	...
	fs.readFile("deckcache/" + deck, function(err, data){
		...
		if (!err){
			...
			if (mark !== undefined){
				...
				ret += "<rect class='"+cls+"' width='16' height='160'/><text x='5' y='-5' transform='rotate(90)'>"+etg.eleNames[mark]+"</text>";
				addClass(cls, "fill:"+elecols[mark]);
			}
			ret = "<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' height='160'"+" width='"+(y?x+100:x)+"'><style type='text/css'><![CDATA[text{font-size:12px}"+classString()+"]]></style>" + ret + "</svg>";
			res.writeHead(200, {"Content-Type": "image/svg+xml"});
			res.end(ret);
