...
function sockEvent(name, func){
	sockEvents[name] = func;
}
...
userEvent("passchange", function(data, user){
	...
	if (!pass.length){
		...
		delete user.iter;
		user.auth = user.name;
		this.emit("passchange", user.name);
	}else{
		var socket = this;
...
userEvent("chat", function (data) {
	if (data.to) {
		...
		if (usersock[to]) {
			...
			this.emit("chat", { msg: data.msg, mode: "blue", u: "To " + to });
		}
		else this.emit("chat", { mode: "red", msg: to + " is not here right now." });
	}
	else{
		delete data.a;
...
userEvent("booster", function(data, user) {
	...
	if (bound || user.gold >= pack.cost) {
		...
		this.emit("boostergive", { cards: newCards, accountbound: bound, cost:pack.cost, packtype:data.pack });
	}
});
foeEcho("endturn");
foeEcho("cast");
...
sockEvent("pvpwant", function(data) {
...
sockEvent("librarywant", function(data){
	if (data.f in users){
